...
#include<string.h>
#include<stdlib.h>
int merge(int* Array_1,int p_lower_1,int q_mid_1,int r_high_1);
int merge_sort_func(int* Array, int p_lower, int r_higher);
...
int main(){
    ...
    int inv_output=0;
    inv_output=merge_sort_func(A,0,n-1);
 ...
 int merge_sort_func(int* Array, int p_lower, int r_higher){
    ...
    inv_comp=inv_comp+merge_sort_func(Array,p_lower,mid_index);
    inv_comp=inv_comp+merge_sort_func(Array,mid_index+1,r_higher);
    inv_comp=inv_comp+merge(Array,p_lower,mid_index,r_higher);
    }
 ...
 int merge(int* Array_1,int p_lower_1,int q_mid_1,int r_high_1){
   ...
   P[end_index_1]=99999999;
   Q[end_index_2]=99999999;
 ...
 for(h=p_lower_1;h<=r_high_1;h++)
    {count_comparision=count_comparision+1;
      ...
      else{inv_func=inv_func+(end_index_1-i);
          Array_1[h]=Q[j];
          j++;
           }
    }
